...
$string['changepublishstatuscat'] = '<a href="{$a->caturl}">A categoria "{$a->name}"</a> na disciplina "{$a->coursename}" terá seu status de compartilhamento trocado de <strong>{$a->changefrom} para {$a->changeto}</strong>.';
...
$string['deletecoursecategorywithquestions'] = 'Existem questões deste banco associadas à esta categoria de disciplina. Se você continuar, serão apagadas. Use a interface de administração do banco de questões para transferir as questões.';
...
*Contexto da categoria de disciplina - Questões disponíveis em todos os módulos de atividade de todas as disciplinas daquela categoria
...
$string['questionaffected'] = '<a href="{$a->qurl}">Questão "{$a->name}" ({$a->qtype})</a> está nesta categoria de questão mas também está na <a href="{$a->qurl}">questão "{$a->quizname}"</a> na disciplina "{$a->coursename}".';
...
$string['questionsmovedto'] = 'Questões ainda utilizadas movidas para "{$a}" na categoria de disciplina correspondente.';
...
$string['unusedcategorydeleted'] = 'Esta categoria foi cancelada porque depois do cancelamento da disciplina as questões não foram mais usadas.';
